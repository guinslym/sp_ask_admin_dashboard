...
class Chats(object):
    ...
    def __init__(self, chat):
        ...
        self.chat_standalone_url = "https://ca.libraryh3lp.com/dashboard/queues/{0}/calls/REDACTED/{1}".format(self.queue_id, self.chat_id)
...
def retrieve_transcript(transcript_metadata, chat_id):
    ...
    get_transcript = transcript_metadata['transcript'] or '<div>No transcript found</div>'
    ...
    for div in divs[1::]:
        try:
            transcript.append(
                {'chat_id':chat_id, 'message':str(div), 
                'counter':counter, 
                'chat_standalone_url':  "https://ca.libraryh3lp.com/dashboard/queues/{0}/calls/REDACTED/{1}".format(queue_id, chat_id) ,
                'guest': guest
                })
            counter += 1
        except:
...
def helper_for_operator_assignments():
    ...
    for user in users.get_list():
        ...
        for assignment in assignments[0:5]:
            assignment['school'] = find_school_by_queue_or_profile_name(assignment.get('queue'))
            operator_report.append(assignment)

    assignments = [{'queue': assign.get('queue'), "operator": assign.get('user'), 'school': assign.get('school')} for assign in operator_report]
    return assignments
