...
def chat_usage_per_month_for_a_past_year(client, a_given_year):
    ...
    chats_for_that_time = [{'year': parse(chat.get('date')).year, 'month':parse(chat.get('date')), 'month_name':parse(chat.get('date')).strftime("%b"),  'date':chat.get('date'), 'count':chat.get('count')}  for chat in chats_for_that_time]
    #stock last academic year in sessions (if session ACADEMIC do this... )

    given_year_usage_per_month = list()
    for element in range(1,13):
        this_month = [item.get('count', 0) for item in chats_for_that_time if item.get('month').month==element]
        given_year_usage_per_month.append(sum(this_month))
    return given_year_usage_per_month


def get_url(queue):
    url = "https://ca.libraryh3lp.com/presence/jid/"+queue+"/chat.ca.libraryh3lp.com/text"
...
def get_homepage(request, *args, **kwargs):
    ...
    client = Client()
    chats = client.chats()
    to_date = str(today.year) + "-" + '{:02d}'.format(today.month)  + "-" + str(today.day)
    chats = chats.list_day(year=last2Days.year, month=last2Days.month, day=last2Days.day, to=to_date)
    ...
    if request.is_ajax():
        ...
        'last_year':today.year - 1, 'this_year':today.year,
        }, safe=False)
...
def get_total_chats_per_month_for_this_year(request):
    ...
    return JsonResponse({"this_year_usage_per_month":this_year_usage_per_month}, safe=False)
...
def get_total_for_this_day(request):
    client = Client()
    today = datetime.today()
    total_chat_today = client.chats().list_day(year=today.year, month=today.month, day=today.day)
...
def get_data_for_chart(request):
    ...
    return JsonResponse({'last_year':chats_last_year_per_month,
